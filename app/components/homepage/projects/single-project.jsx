...
const SingleProject = ({ project }) => {
  ...
  return (
    <div className='group w-full h-fit flex flex-col items-center justify-center relative cursor-text overflow-hidden px-3 md:px-8 py-[1.4rem] bg-black/80 shadow-elegant rounded-lg border border-purple/30 hover:border-pink/50 transition-all duration-300'
    >
      ...
      <div className='flex flex-col items-center justify-between w-full h-full'>
        <h2 className='text-white not-italic font-semibold text-[1.525rem] leading-[110%] text-center capitalize'>
          {name}
        </h2>
        <div className="p-6">
          <Image
        ...
        <div className="flex items-center justify-between w-full">
          <Link
            href={demo}
            target='_blank'
            className="flex justify-center items-center w-10 h-10 rounded-full border-2 border-white/30 text-white transition-all duration-300 hover:bg-purple/20 hover:text-pink hover:border-pink hover:scale-110 decoration-clone cursor-pointer no-underline delay-[0.3s]">
            <FaPlay />
          </Link>
          ...
          <Link
            href={code}
            target='_blank'
            className="flex justify-center items-center w-10 h-10 rounded-full border-2 border-white/30 text-white transition-all duration-300 hover:bg-purple/20 hover:text-pink hover:border-pink hover:scale-110 cursor-pointer no-underline delay-[0.3s] group-hover:translate-x-[-140px]">
            <FaCode />
          </Link>
        </div>
      </div>
      <p className="absolute w-[90%] md:w-[85%] md:min-h-[150px] translate-x-[-110%] transition-transform duration-[0.9s] p-6 leading-[110%] rounded-[0_20px_20px_0] left-0 top-0 bg-black/90 text-white translate-y-[25%] md:translate-y-[50%] group-hover:translate-x-[-2%] text-xs md:text-sm border border-purple/30">
        {description}
      </p>
      <div className='group-hover:translate-x-0 absolute w-[140px] text-[0.8rem] flex justify-center gap-2 flex-col translate-x-full transition-transform duration-[0.5s] delay-[0.3s] p-[0.825rem] rounded-[10px_0_0_10px] right-0 bottom-4 bg-black/90 text-white border border-purple/30'>
        {tags.map((tag, id) => (
